...
import { assessSiteRisk } from '../services/apiService'
...
export function useSiteRisk() {
  ...
  const assessSite = useCallback(async (lat, lng) => {
    setLoading(true)

    try {
      // Try backend API first
      const data = await assessSiteRisk(lat, lng)
      setResult({ ...data, site: { lat, lng } })
    } catch (error) {
      console.warn('[Terra AI] Backend unavailable, using local fallback:', error.message)
      // Fallback to local mock data
      _localFallback(lat, lng, setResult)
    } finally {
      setLoading(false)
    }
  }, [])
 ...
 * Uses the same logic as the original mock implementation.
 */
function _localFallback(lat, lng, setResult) {
